# Руководство по установке и работе с шаблоном

## Необходимое ПО

- VS Code или любой другой редактор кода
- Node.js версии 20.15, скачать и установить ее можно с официального сайта, однако обратите внимание на скачиваемую версию, могут возникнуть ошибки.

## Начало работы

- Для копирования шаблона к себе на аккаунт github вам необходимо нажать кнопку fork на странице данного репозитория.

- После этого перейдя в свой скопированный репозиторий вы сможете скопировать репозиторий на свой пк.
  ### Для этого: 
  - создайте папку в удобном для вас месте
  - откройте папку при помощи vs code
  - создайте терминал
  - введите команду `git clone <https ссылка на ваш репозиторий из кнопки clone> .` !! не пропустите точку, это заставит репозиторий развернуться в нынешней папке, а не создавать внутреннюю
  - может потребоваться ваш логин пароль от github или же вас просто перекинет в браузер для авторизации
  - ваш репозиторий скопирован и можете продолжать работу

## Для работы используйте такие команды

- Для работы с командами далее вам понадобится терминал, который можно создать из верхнего меню VS code

- Для установки всех зависимостей: `$ npm install`. Данной командой вы установите все необходимые библиотеки для работы шаблона;
- Для запуска сборщика Gulp нужно использовать: `$ npm run dev`. Данной командой вы запустите свой проект в режиме разработки, где весь код будет доступен для просмотра из браузера;
